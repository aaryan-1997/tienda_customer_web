    ...
    child: InkWell(
      onTap: ()async{
        ...
        if(_formKey.currentState!.validate()){
          Provider.of<AuthProvider>(context,listen: false).otp(context: context, data: {
            "otp_text":_pinController.text
          }).then((value) {
            setState(() {
              isLoad=false;
            });
            Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context)=>StoreScreen(),), (route) => false);
          });
        }else{
          log('message');
          setState(() {
